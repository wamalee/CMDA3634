...
int main (int argc, char **argv) {
  ...
  if (rank == 0) {
     printf("Alice is setting up the encryption\n");
  ...
  printf("Let's broadcast the messages! rank %u\n", rank);
    
  MPI_Bcast(message, Nmessages, MPI_UNSIGNED, 1, MPI_COMM_WORLD);
  MPI_Bcast(a, Nmessages, MPI_UNSIGNED, 1, MPI_COMM_WORLD);
  
  printf("Rank %u has recived the message\n", rank);  
